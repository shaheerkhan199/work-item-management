...
let HistoryService = (() => {
    ...
    let _classThis;
    var HistoryService = _classThis = class {
        constructor(prisma) {
            this.prisma = prisma;
        }
        async recordEvent(eventDto) {
        ...
        async getWorkItemHistory(workItemId, userId, userRole) {
            // Verify work item exists and check authorization
            const workItem = await this.prisma.workItem.findUnique({
                where: { id: workItemId },
            });
            if (!workItem) {
                throw new common_1.NotFoundException(`Work item ${workItemId} not found`);
            }
            // VIEWER and OPERATOR can only access history for their own work items
            if (userRole !== "ADMIN" && workItem.createdById !== userId) {
                throw new common_1.ForbiddenException("You do not have access to this work item's history");
            }
